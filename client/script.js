...
// CODE HERE
function changeClrBack() {
    sayHelloButton.style.backgroundColor = '';
    sayHelloButton.style.color = '';
}

sayHelloButton.addEventListener('mouseout', changeClrBack);
// PROBLEM 4
/*
...
const ohMy = () => {
    ...
    }).catch(err => console.log({err, message:'error with the ohmy function'}));
}
...
const repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/repeat').then((res) => {
        console.log(res.data);
        document.querySelector('#repeat-text').textContent = res.data;
        document.querySelector('#repeat-text').style.display = 'block';
    }).catch(err => console.log({err, message:'error with the repeat param function'}));
}
...
// CODE HERE
const makeReq = () => {
    axios.get('http://localhost:3000/query-test?name=joe&age=65]').then((res) => {
    console.log(res.data);
    }).catch(err => console.log({err, message:'error with the query function'}));
}

document.querySelector('#query-button').addEventListener('click', makeReq);
////////////////
//INTERMEDIATE//
////////////////
...
const createFood = (e) => {
    ...
    axios.post(`http://localhost:3000/food`, body).then(res => {
        console.log(res.data);
        let newP = document.createElement('p');
        newP.textContent = res.data[res.data.length - 1];
        document.body.appendChild(newP);
    }).catch(err => console.log({err, message:'error with the createFood function'}));
